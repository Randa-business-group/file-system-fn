          ...
          {PROBLEM_POINTS.map((point, index) => {
            ...
            return (
              <AnimatedSection
                ...
                duration={600}
              >
                <article className="group relative h-full rounded-lg border border-default bg-surface p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md sm:p-8">
                  {/* Icon */}
                  <span className="flex h-12 w-12 items-center justify-center rounded-xl bg-red-50 text-error transition-transform duration-300 group-hover:scale-105">
                    <Icon className="h-6 w-6" />
                  </span>

                  {/* Title */}
                  <h3 className="mt-5 text-lg font-semibold text-foreground transition-colors group-hover:text-error">
                    {point.title}
                  </h3>
